...
# .groupby('Launch Site')['class'].value_counts().reset_index()
...
def get_scatter_plot(entered_site, mass_range):
    filtered_df = spacex_df.loc[(spacex_df['Payload Mass (kg)'] >= mass_range[0]) & (spacex_df['Payload Mass (kg)'] <= mass_range[1])] 
    if entered_site == 'ALL':
